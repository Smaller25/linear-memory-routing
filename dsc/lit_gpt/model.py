...
try:
    ...
    # flash_attn is only required for attention/SWA blocks. Pure recurrent GDN-2 models
    # (gdn2_per_layer=1) do not need it.
...
from .dsc import DSCGatedDeltaNet2
from mc_gdn2.layer import MemoryCachingGDN2Layer
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        if not use_kv_cache:
            if getattr(self.config, "block_activation_checkpoint", False):
                from torch.utils.checkpoint import checkpoint as _ckpt
                for block in self.transformer.h:
                    def _run_block(_blk, _x, _rope, _msl):
                        h, _ = _blk(_x, _rope, _msl)
                        return h
                    x = _ckpt(_run_block, block, x, rope, max_seq_length, use_reentrant=False)
            else:
                for block in self.transformer.h:
...
class Block(nn.Module):
    def __init__(self, config: Config, layer_idx: int) -> None:
        ...
        # DSC: when config.dsc=True (set explicitly for dsc-only configs), every GDN-2 layer
        # becomes a DSCGatedDeltaNet2. Vanilla GDN-2 path is unchanged.
        use_dsc = getattr(config, "dsc", False)
        # MC: every GDN-2 layer is wrapped with the configured paper aggregator
        # (SSC, GRM, or linear-memory-equivalent Memory Soup).
        use_mc = getattr(config, "mc", False)
        # Log-Linear GDN-2: preserve the GDN-2 hidden transition and replace
        # its single temporal state with a weak Fenwick hierarchy.
        use_log_linear_gdn2 = getattr(config, "log_linear_gdn2", False)
        if sum((bool(use_dsc), bool(use_mc), bool(use_log_linear_gdn2))) > 1:
            raise ValueError(
                "dsc, mc, and log_linear_gdn2 are mutually exclusive "
                "GDN-2 variants"
            )
        gdn_kw = {}
        if getattr(config, "gdn_num_heads", None) is not None:
            gdn_kw["num_heads"] = config.gdn_num_heads
        if getattr(config, "gdn_head_dim", None) is not None:
            gdn_kw["head_dim"] = config.gdn_head_dim
        if self.use_gdn2:
            if use_dsc:
                self.attn = DSCGatedDeltaNet2(hidden_size=config.n_embd)
                # Override defaults from config if set.
                self.attn.dsc_chunk_size = getattr(config, "dsc_chunk_size", 256)
                self.attn.dsc_topk = getattr(config, "dsc_topk", 2)
            elif use_mc:
                base = GatedDeltaNet2(hidden_size=config.n_embd, **gdn_kw)
                mc_variant = getattr(config, "mc_variant", "ssc")
                if mc_variant == "ssc":
                    # Keep the validated SSC-v2 construction path unchanged.
                    self.attn = MemoryCachingGDN2Layer(
                        base,
                    ...
                    # independent from the new dense implementation.
                    from mc_gdn2.dense_layer import (
                        DenseMemoryCachingGDN2Layer,
                    )

                    self.attn = DenseMemoryCachingGDN2Layer(
                        base,
                        variant=mc_variant,
                        chunk_size=getattr(config, "mc_chunk_size", 256),
                        checkpoint_mode="independent",
                        route_block_size=getattr(config, "mc_route_block_size", 16),
                    )
            elif use_log_linear_gdn2:
                # Lazy import keeps the validated vanilla/SSC-v2 paths
                # independent from the new hierarchy implementation.
                from log_linear_gdn2.layer import LogLinearGDN2Layer

                self.attn = LogLinearGDN2Layer(
                    GatedDeltaNet2(hidden_size=config.n_embd, **gdn_kw),
                    max_sequence_length=config.block_size,
                    lambda_mode=getattr(
                        config, "log_linear_lambda_mode", "positive"
                    ),
                    checkpoint_levels=getattr(
                        config, "log_linear_checkpoint_levels", True
                    ),
                )
            else:
                self.attn = GatedDeltaNet2(hidden_size=config.n_embd, **gdn_kw)
        else:
