"""MC SSC v4 pretrain — v3 kernels (bit-exact) + training infra optimizations.

Diffs from pretrain.py (v3):
  1. FSDPStrategy(forward_prefetch=True): overlap next layer's all-gather with
     current layer's forward. In torch 2.12, gradient_as_bucket_view/static_graph
     are no longer exposed (always-on or removed). backward_prefetch defaults to
     BACKWARD_PRE.
  2. In-training profiler integrated: samples 5 iters every 200, dumps
     torch.profiler table + per-component (fwd/bwd/optimizer) cuda.Event
     timing to log_dir/v4_profile/.
  3. Per-iter cuda.Event timing (always on) → break down fwd vs bwd vs
     optimizer. Logged every 50 iters via [v4-perf] tag.

Kernel is identical to v3 (mc_v3.cached_memory_read_v3c). Realistic v4 iter ≈
v3 (855ms) + small savings from forward_prefetch. The bulk of the gap-to-vanilla
(460ms MC overhead) is in the MC layer ops OUTSIDE the gather kernel and would
require a fused-CUDA v5 to close further.
...
# v4 lives at dsc/mc_v4/pretrain_v4.py. Add both long-gdn (for `from dsc.X` imports)
# and dsc (for `from lit_gpt.X` and `from mc_v4.X` imports) to sys.path.
_file_dir = Path(__file__).parent.resolve()           # dsc/mc_v4
_dsc_dir = _file_dir.parent.resolve()                  # dsc
_repo_root = _dsc_dir.parent.resolve()                 # long-gdn
for _p in (str(_repo_root), str(_dsc_dir)):
    if _p not in sys.path:
        sys.path.append(_p)
wd = _repo_root  # backward-compat with code below that uses `wd`
...
from mc_v4.in_training_profiler import InTrainingProfiler

_TRAIN_START_TIME = time.time()

# Per-iter cuda.Event timing (always on) — measures forward, backward, optimizer.step.
# Lighter than InTrainingProfiler (which samples). Used to get per-iter breakdown.
class IterComponentTimer:
    """Tracks rolling mean of fwd/bwd/optimizer times via cuda.Event."""
    def __init__(self, log_every: int = 50, log_to=None):
        self.log_every = log_every
        self.log_to = log_to or print
        self.times = defaultdict(list)  # name -> list of ms (recent window)
    ...
    def flush_iter(self, iter_num: int):
        """After iter (with optimizer.step or not), compute times for this iter
        and append to history. Synchronize once."""
        torch.cuda.synchronize()
        for name, (s, e) in self._events.items():
            self.times[name].append(s.elapsed_time(e))
        self._events.clear()

    def maybe_log(self, iter_num: int):
        if iter_num % self.log_every != 0 or iter_num == 0:
            return
        if not self.times:
            return
        parts = []
        for name in ("data", "fwd", "bwd", "optim"):
            if name in self.times and self.times[name]:
                arr = self.times[name][-self.log_every:]
                mean = sum(arr) / len(arr)
                parts.append(f"{name}={mean:.1f}ms")
        if parts:
            self.log_to(f"  [v4-perf] iter {iter_num}: {' '.join(parts)}")
...
def main(args):
    ...
    if args.interactive_job:
        strategy = FSDPStrategy(
            ...
            # v4 opt: overlap next layer's all-gather with current layer's forward.
            # In torch 2.12, gradient_as_bucket_view/static_graph are not exposed
            # (always-on or removed). backward_prefetch defaults to BACKWARD_PRE.
...
def train(args, _TRAIN_START_TIME, fabric, state, train_dataloader, val_dataloader, monitor, resume):
    ...
    # v4: in-training profiler + per-iter component timer
    profile_dir = os.path.join(args.out_dir, "v4_profile")
    in_prof = InTrainingProfiler(
        log_dir=profile_dir,
        sample_period_iters=200,
        sample_iters=5,
        warmup_iters=100,
        rank=fabric.global_rank,
    )
    comp_timer = IterComponentTimer(log_every=50, log_to=fabric.print)
    if fabric.global_rank == 0:
        fabric.print(f"[v4] in-training profiler → {profile_dir}")
        fabric.print(f"[v4] FSDP strategy: forward_prefetch=True (rest: torch defaults)")
    ...
    for train_data in train_dataloader:
        ...
        # v4: profiler tick (manages sampling windows)
        in_prof.tick(state["iter_num"])
        ...
        # v4: gate the per-iter cuda.Event timing entirely on sample iters.
        # Creating events + record() every iter is cheap (async), but skipping
        # the context managers entirely eliminates all overhead on non-sample
        # iters. Sample 1 in 50 → flush + log on those iters only.
        should_profile = (state["iter_num"] % 50 == 0)
        cm_fwd = comp_timer.time("fwd") if should_profile else nullcontext()
        cm_bwd = comp_timer.time("bwd") if should_profile else nullcontext()
        cm_optim = comp_timer.time("optim") if should_profile else nullcontext()
        ...
        # v4: sample-only flush — cuda.synchronize() every iter would add ~110ms
        # overhead and kill async pipelining. Flush only on sample iters.
        if should_profile:
            comp_timer.flush_iter(state["iter_num"])
            comp_timer.maybe_log(state["iter_num"])
