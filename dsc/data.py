...
class StatefulStreamingDataset(IterableDataset):
    def __init__(
        ...
        # B-4 fix: the chunk-buffer shuffle generator used to be seeded with
        # epoch+rank only, ignoring the run's --seed entirely, so "different
        # seed" runs saw identical data order. seed=0 reproduces the old
        # behavior; pretrain.py now passes its --seed here.
        self.seed = seed
        if buffer_size == -1:
    ...
    def __iter__(self):
        g = torch.Generator()
        g.manual_seed(int(self.seed) + int(self._epoch) + int(self.rank))
...
def get_stateful_stream_tok_dataset(corpus_name='slimpajama', path=None, split='train', tokenizer=None, block_size=2048, rank=0, world_size=1, batch_size=32, num_workers=8, seed=0):
    ...
    dataset = StatefulStreamingDataset(dataset, tokenizer, context_length=block_size, rank=rank, world_size=world_size, buffer_size=buffer_size, seed=seed)
