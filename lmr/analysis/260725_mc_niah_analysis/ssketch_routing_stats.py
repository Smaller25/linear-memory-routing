"""ssketch: answer position에서 layer별 gold-chunk routing 정확도.

0024 E1의 `routing_stats.py` 를 출발점으로 삼되 **별도 파일로 분리**했다.
`routing_stats.py` 는 0024/0025 결과를 만들어낸 코드 그대로 얼려 둔다 —
`x1_dump.py`, `x2_probe.py`, `fidelity.py` 가 거기서 직접 import 하므로
그 파일을 고치면 이미 발표한 수치의 재현 경로가 함께 바뀐다.
따라서 ssketch 쪽 변경은 전부 이 파일 안에서만 한다.

`routing_stats.py` 와 달라진 점:
  - `--descriptor {meank, ssketch}` arm 축
  - `rank_order()` — stable argsort (0024는 non-stable). 같은 meank 설정이어도
    동점 처리가 달라 수치가 미세하게 갈릴 수 있다. PRIOR 열은 방향 확인용으로만 쓴다.
  - `niah_multivalue` 데이터셋과 `macro_hit2` 보조 DV
  - arm별 shard 출력 (아래)
  - `make_figure` 제거 — 오프라인 `ssketch_table.py` 로 옮김
