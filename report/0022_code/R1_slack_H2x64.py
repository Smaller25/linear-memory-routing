...
class Model(nn.Module):
    def __init__(self,vocab,d=128,H=2,hd=64,L=2,variant="baseline"):
...
def run(variant,steps=2000,seed=0):
    ...
    opt=torch.optim.AdamW(m.parameters(),lr=3e-3,weight_decay=0.1,betas=(0.9,0.95)); tk=[4,8,16]; t0=time.time()
    for step in range(1,steps+1):
        kv=tk[step%len(tk)]; ids,lab=make_mqar(64,V,kv,192,step); ids,lab=ids.to(dev),lab.to(dev)
...
if __name__=="__main__":
    print("RESULT_HEADER variant | recall kv8/16/32 | erank",flush=True)
    for var in ["baseline","C4","C1","C2"]:
        res,er=run(var); print("RESULT %-9s | %.3f / %.3f / %.3f | %.2f"%(var,res[8],res[16],res[32],er),flush=True)
